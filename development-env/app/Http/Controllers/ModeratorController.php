...
class ModeratorController extends Controller
{
    ...
    */
    ...
    public function action(Request $request){
        ...
        if($action==="approve_creation")  return $this->approve_creation($request);
        if($action==="remove_creation")  return $this->remove_creation($request);
        if($action==="approve_modification")  return $this->approve_modification($request);
        if($action==="remove_modification")  return $this->remove_modification($request);
        if($action==="restore_auction")  return $this->restore_auction($request);
        if($action==="remove_auction") return $this->remove_auction($request);
        if($action==="get_new_description") return $this->get_new_description($request);
        //Unkown action error
