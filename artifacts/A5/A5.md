...
Neste artefacto é apresentado o modelo relacional da plataforma Bookhub obtido pelo do mapeamento do modelo conceptual de dados desta mesma plataforma. Este artefacto tem como objetivo a identificação do esquema relacional, e os seus atributos, domínios, chaves primárias (_Primary Keys_) , chaves estrangeiras (_Foreign Keys_) e outras regras de integridade.
...
Para validar o esquema relacional obtido através do modelo conceptual, todas as dependências funcionais estão identificadas e a normalização de dados.
...
Como todos os esquemas de relações estão na forma normal *Boyce–Codd Normal Form* (BCNF), todo o modelo relacional se encontra nesta forma não sendo necessário recorrer à normalização do mesmo.
