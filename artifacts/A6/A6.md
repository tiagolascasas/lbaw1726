...
The following indexes were created with the purpose of greatly enhancing the performance of the most frequent queries. By creating indexes, however, there is always the risk of introducing an extra overhead on updates and inserts, as these may change the internal structure of the indexed data. In our specific case, given that our updates are done in different attributes than the ones used for indexing, there should be no need to reorganize the index upon an update. Furthermore, given that most hash indexes are built upon a numerical and sequential primary key, new inserts on those indexes rarely imply collisions, which means that there isn't any reorganization to do. Therefore, our indexes have little to no overhead on insertions and updates, all the while greatly improving the speed of many queries.
...
<table>
  ...
  <tr>
    <td><strong>Trigger description</strong></td><td>Checks if an image doesn't belong to a member and an auction at the same time</td>
  </tr>
</table>
...
1. Correction of the estimated growth of the relation name member.
2. All indices got revised to make sure they don't create overhead on inserts or updates.
3. Changes to the schema of database, to better follow SQL norms.
4. Conversion of unnecessary triggers to check constraints.
5. Creation of a new trigger with the reference: TRIGGER08.
